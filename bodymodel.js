...
export default class BodyModel {
    ...
    dfdt(baseline, dailyParams) {
        return (1.0 - this.getp()) * (dailyParams.calories - this.getTEE(baseline, dailyParams) - this.carbflux(baseline, dailyParams)) / 9440.0;
    }

    dldt(baseline, dailyParams) {
        return this.getp() * (dailyParams.calories - this.getTEE(baseline, dailyParams) - this.carbflux(baseline, dailyParams)) / 1807.0;
    }
    ...
    avgdt_weighted(wt, bchange) {
        let sumf = 0.0, suml = 0.0, sumg = 0.0, sumdecw = 0.0, sumtherm = 0.0, wtsum = 0;
        ...
        return new BodyChange(sumf / wtsum, suml / wtsum, sumg / wtsum, sumdecw / wtsum, sumtherm / wtsum);
    }
}
