...
class App {
  ...
  async loadCompendium() {
    try {
      const res = await fetch('pa_compendium_2024.json');
      this.compendium = await res.json();
    } catch (e) {
      console.error('Failed to load activity compendium', e);
    }
  }

  addPhase(day = 0, cals = 2000, act = 0, ramp = false, id = null) {
    const list = document.getElementById('intervention-list');
    const template = document.getElementById('lifestyle-phase-template');
    ...
    const div = clone.querySelector('.intervention-box');
    div.dataset.phaseId = phaseId;

    // Set Values & IDs
    div.querySelector('h3').textContent = `Phase ${phaseIdx}`;

    const dayInput = div.querySelector('.phase-day');
    dayInput.value = day;

    const calsInput = div.querySelector('.phase-cals');
    calsInput.id = calsId;
    calsInput.value = cals;
    div.querySelector('.unit-energy').textContent = this.isCalories ? 'cals/day' : 'kj/day';

    const actInput = div.querySelector('.phase-act');
    actInput.id = actId;
    actInput.value = act;

    const estBtn = div.querySelector('.estimate-phase-act');
    estBtn.dataset.target = actId;
    estBtn.addEventListener('click', (e) => this.openActChangeModal(e.target.dataset.target));

    const rampInput = div.querySelector('.phase-ramp');
    rampInput.checked = ramp;
    ...
    div.querySelectorAll('input').forEach((input) => {
      input.addEventListener('change', (e) => {
        ...
        if (e.target.classList.contains('phase-cals')) type = 'sodium';
  ...
  validateInput(id, val) {
    ...
    switch (id) {
      case 'age':
        ...
        if (numVal > 120) {
          error = 'Age must be no more than 120';
          newVal = 120;
        }
        break;
      case 'weight':
      case 'goal-weight':
        if (numVal < 10) {
          error = 'Weight must be at least 10kg';
          newVal = 10;
        }
        if (numVal > 1000) {
          error = 'Weight must be no more than 1000kg';
          newVal = 1000;
        }
        break;
      case 'height':
        if (numVal < 50) {
          error = 'Height must be at least 50cm';
          newVal = 50;
        }
        if (numVal > 300) {
          error = 'Height must be no more than 300cm';
          newVal = 300;
        }
        break;
      case 'height-ft':
      ...
      case 'pal':
        if (numVal < 1.1) {
          error = 'PAL must be at least 1.1';
          newVal = 1.1;
        }
        if (numVal > 3.0) {
          error = 'PAL must be no more than 3.0';
          newVal = 3.0;
        }
        break;
      case 'goal-days':
      case 'sim-length':
        ...
        if (numVal > 3650) {
          error = 'Max 10 years (3650 days)';
          newVal = 3650;
        }
        if (numVal > 500) {
          error = 'Warning: Long simulations may be slow.';
        }
        break;
      case 'activity-change':
      case 'phase-act':
        if (numVal < -100) {
          error = 'Min change -100%';
          newVal = -100;
        }
        if (numVal > 500) {
          error = 'Max change 500%';
          newVal = 500;
        }
        break;
      case 'carbs-pct':
      ...
      case 'sodium':
        if (numVal < 0) {
          error = 'Sodium cannot be negative';
          newVal = 0;
        }
        if (numVal > 20000) {
          error = 'Max sodium 20,000mg';
          newVal = 20000;
        }
        break;
      case 'bfp':
        if (numVal < 0) {
          error = 'BFP cannot be negative';
          newVal = 0;
        }
        if (numVal > 100) {
          error = 'Max BFP 100%';
      ...
      case 'rmr':
        if (numVal < 0) {
          error = 'RMR cannot be negative';
          newVal = 0;
        }
        if (numVal > 100000) {
          error = 'Max RMR 100,000 cals';
          newVal = 100000;
        }
        break;
    }
  ...
  setPalMode(mode) {
    this.palMode = mode;
    const toggle = document.getElementById('pal-mode-toggle');
    toggle?.querySelectorAll('button').forEach((btn) => {
      btn.classList.toggle('active', btn.dataset.mode === mode);
    });
    document.getElementById('pal-simple-container')?.classList.toggle('hidden', mode !== 'simple');
    document
      .getElementById('pal-advanced-container')
      ?.classList.toggle('hidden', mode !== 'advanced');

    if (mode === 'advanced') {
      const list = document.getElementById('pal-advanced-list');
      if (list && list.children.length === 0) {
        this.addPalAdvancedRow();
      }
    }
  }

  addPalAdvancedRow(initialData = null) {
    const list = document.getElementById('pal-advanced-list');
    const template = document.getElementById('pal-advanced-row-template');
    if (!list || !template) return;

    const clone = template.content.cloneNode(true);
    const row = clone.querySelector('.activity-row');
    const searchInput = row.querySelector('.activity-search');
    const select = row.querySelector('.activity-select');

    if (initialData) {
      searchInput.value = initialData.description || '';
      const opt = document.createElement('option');
      opt.value = initialData.met;
      opt.textContent = initialData.description;
      opt.selected = true;
      select.appendChild(opt);
      row.querySelector('.act-duration').value = initialData.duration;
      row.querySelector('.act-frequency').value = initialData.frequency;
      row.querySelector('.act-period').value = initialData.period;
    }
    ...
    searchInput.addEventListener('input', (e) => {
      ...
      const matches = this.compendium
        .filter(
          (act) =>
            act.description.toLowerCase().includes(query) ||
            act.heading.toLowerCase().includes(query)
        )
        .slice(0, 50);
      ...
      if (matches.length > 0) {
        ...
        matches.forEach((act) => {
          const opt = document.createElement('option');
          opt.value = act.met;
          opt.textContent = `[${act.met} MET] ${act.description}`;
          select.appendChild(opt);
        });
    ...
    select.addEventListener('change', () => {
      ...
      select.classList.add('hidden');
      this.updateAdvancedPalPreview();
    });

    // Hide select when clicking outside
    document.addEventListener('click', (e) => {
      if (!row.contains(e.target)) select.classList.add('hidden');
    });

    row.querySelectorAll('input, select').forEach((el) => {
      el.addEventListener('change', () => this.updateAdvancedPalPreview());
    });
  ...
  updateAdvancedPalPreview() {
    const pal = this.calculateAdvancedPal();
    const display = document.getElementById('pal-advanced-preview');
    if (display) display.textContent = pal.toFixed(2);
  }
  ...
  populatePalOptions() {
    ...
    Object.values(WorkActivityLevel)
    ...
    Object.values(LeisureActivityLevel)
      .sort((a, b) => a.sortOrder - b.sortOrder)
      .forEach((opt) => {
        const el = document.createElement('option');
        el.value = opt.name;
    ...
    const updateDescs = () => {
      document.getElementById('work-desc').textContent =
        workSelect.selectedOptions[0]?.dataset.desc || '';
      document.getElementById('leisure-desc').textContent =
        leisureSelect.selectedOptions[0]?.dataset.desc || '';
    };
  ...
  initEventListeners() {
    ...
    document
      .getElementById('toggle-advanced-show')
      ?.addEventListener('click', () => this.setAdvanced(true));
    document
      .getElementById('toggle-advanced-hide')
      ?.addEventListener('click', () => this.setAdvanced(false));

    document.getElementById('tab-goal')?.addEventListener('click', () => this.setTab('goal'));
    document
      .getElementById('tab-lifestyle')
      ?.addEventListener('click', () => this.setTab('lifestyle'));
    document
    ...
    const palToggle = document.getElementById('pal-mode-toggle');
    palToggle?.querySelectorAll('button').forEach((btn) => {
      btn.addEventListener('click', () => {
        this.setPalMode(btn.dataset.mode);
      });
    });

    document
      .getElementById('add-pal-advanced-row')
      ?.addEventListener('click', () => this.addPalAdvancedRow());
    ...
    document.getElementById('btn-estimate-maint-act-change')?.addEventListener('click', () => {
      this.openActChangeModal('maint-activity-change');
    });
    document
      .getElementById('pal-change-cancel')
      ?.addEventListener('click', () => changeModal.close());
    document
      .getElementById('add-activity-row')
      ?.addEventListener('click', () => this.addActivityRow());
    changeModal?.addEventListener('submit', () => {
      ...
      if (targetEl) {
        ...
        if (targetId === 'activity-change') {
          const maintAct = document.getElementById('maint-activity-change');
          if (maintAct && !maintAct.dataset.touched) {
            maintAct.value = targetEl.value;
          }
        }
        this.updateResults();
        this.saveToLocalStorage();
    ...
    // Sync maintenance activity with goal activity by default
    document.getElementById('activity-change')?.addEventListener('input', (e) => {
      const maintAct = document.getElementById('maint-activity-change');
      if (maintAct && !maintAct.dataset.touched) {
        maintAct.value = e.target.value;
        this.updateResults();
      }
    });
    document.getElementById('maint-activity-change')?.addEventListener('input', (e) => {
  ...
  setAdvanced(show, shouldSave = true) {
    ...
    document
      .querySelectorAll('.advanced-controls')
  ...
  saveToLocalStorage() {
    ...
    const palAdvancedList = document.getElementById('pal-advanced-list');
    const palActivities = [];
    if (palAdvancedList) {
      Array.from(palAdvancedList.children).forEach((row) => {
        palActivities.push({
          description: row.querySelector('.activity-search')?.value,
    ...
    const data = {
      ...
      maintActivityChange: document.getElementById('maint-activity-change')?.value,
  ...
  loadFromLocalStorage() {
    ...
    try {
      ...
      if (data.palMode) {
        this.setPalMode(data.palMode);
      }
      if (data.palActivities && Array.isArray(data.palActivities)) {
        const list = document.getElementById('pal-advanced-list');
        if (list) {
          list.innerHTML = '';
          data.palActivities.forEach((act) => {
            this.addPalAdvancedRow(act);
          });
        }
      }
      ...
      if (data.activityChange)
        document.getElementById('activity-change').value = data.activityChange;
      if (data.maintActivityChange)
        document.getElementById('maint-activity-change').value = data.maintActivityChange;
  ...
  applyEnergyUnitUI() {
    ...
    document
      ...
      .forEach((el) => (el.textContent = this.isCalories ? 'cals/day' : 'kj/day'));
  }
  ...
  setTab(tab) {
    ...
    document.getElementById('goal-section')?.classList.toggle('hidden', tab !== 'goal');
    document.getElementById('lifestyle-section')?.classList.toggle('hidden', tab !== 'lifestyle');

    document.getElementById('goal-results-group')?.classList.toggle('hidden', tab !== 'goal');
    document
      .getElementById('lifestyle-results-group')
      ?.classList.toggle('hidden', tab !== 'lifestyle');
  ...
  setUnits(isMetric) {
    ...
    if (isMetric) {
      ...
      document.getElementById('goal-weight').value = (currentGoalWeight * 2.20462 || 154.3).toFixed(
        1
      );
      if (currentHeight) {
  ...
  updateResults() {
    try {
      ...
      this.baseline.isMale = sex;
      this.baseline.weight = weight;
      this.baseline.age = age;
      this.baseline.height = height;
      this.baseline.pal = pal;
      ...
      if (bfpModeEl) {
        ...
        if (!this.baseline.bfpCalc)
          this.baseline.bfp = parseFloat(document.getElementById('bfp')?.value) || 18;
        if (this.baseline.bfpCalc) {
      ...
      if (rmrModeEl) {
        ...
        if (!this.baseline.rmrCalc)
          this.baseline.rmr = parseFloat(document.getElementById('rmr')?.value) || 1700;
        if (this.baseline.rmrCalc) {
  ...
  handleGoalSimulation(simLength, calorieSpread) {
    ...
    const maintActivityChange =
      parseFloat(document.getElementById('maint-activity-change')?.value) || 0;

    try {
      const goalInt = Intervention.forgoal(
        this.baseline,
        goalWeight,
        goalDays,
        activityChange,
      ...
      if (goalInt.calories < 1000) {
        this.showError(
          'goal-cals',
          'Intake below 1000 calories/day may not meet nutritional needs.'
        );
      } else {
      ...
      const trajectory = this.collectTrajectoryData(
        this.baseline,
        goalInt,
        maintInt,
        goalDays,
      ...
      const upperTraj = this.collectTrajectoryData(
        this.getModifiedBaseline(calorieSpread),
        goalInt,
        maintInt,
        goalDays,
        simLength
      );
      const lowerTraj = this.collectTrajectoryData(
        this.getModifiedBaseline(-calorieSpread),
        goalInt,
        maintInt,
        goalDays,
      ...
      if (e.message.includes('Unachievable Goal')) {
        this.showError(
          'goal-weight',
          `This goal is unachievable in ${goalDays} days. Try more time or more activity.`
        );
      }
      this.clearResults();
    }
  ...
  handleLifestyleSimulation(simLength, calorieSpread) {
    ...
    if (lowCalPhase) {
      this.showError(
        'final-weight',
        'One or more phases have an intake below 1000 calories/day, which may not meet nutritional needs.'
      );
    } else {
    ...
    if (interventions.length === 0) {
      interventions.push(
        new Intervention(0, 2000, this.baseline.carbIntakePct, 0, this.baseline.sodium)
      );
    }

    const trajectory = this.collectMultiTrajectoryData(this.baseline, interventions, simLength);
    const upperTraj = this.collectMultiTrajectoryData(
      this.getModifiedBaseline(calorieSpread),
      interventions,
      simLength
    );
    const lowerTraj = this.collectMultiTrajectoryData(
      this.getModifiedBaseline(-calorieSpread),
      interventions,
      simLength
    );
